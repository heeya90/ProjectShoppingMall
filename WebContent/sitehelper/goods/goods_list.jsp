...
<div id="content">
	<div class="admin_loca">HOME - 상품관리 - 상품리스트 - <B>[<span class="productListTitle"></span>]</B>
	</div>
	...
	<div id="productListForm">
		...
		<div class="admin_tsearch">
			...
			</select><input type="hidden" value="" /> ~ <select name="eYear" id="eYear"
				class="selectBox" style="width: 55px;">
			</select><input type="hidden" value="" /> <select name="eMonth" id="eMonth"
				class="selectBox" style="width: 55px;">
			</select><input type="hidden" value="" /> <select name="eDay" id="eDay"
				class="selectBox" style="width: 55px;">
			</select><input type="hidden" value="" /> <select name="pPageSize"
				class="selectBox" style="width: 50px;">
			...
			</select> <input type="button" name="searchButton" id="searchButton"
				value="검 색" class="clickbutton02" />
		</div>
		<div class="admin_arr_cen">
			<span class="admin_arr01">
				<!--label><input type="radio" name="productSort" value="inputday_asc" />등록순↑</label-->
				<label><input type="radio" name="productSort"
					value="categoryC_asc" />카테고리↑</label> <!--label><input type="radio" name="productSort" value="price1_asc" />가격순↑</label-->
				<!--label><input type="radio" name="productSort" value="orderCont_asc" />매출↑</label-->
				<!--label><input type="radio" name="productSort" value="priceUpdateDay_asc" />단가수정↑</label-->
				<label><input type="radio" name="productSort" value="inputday_desc" checked="checked" />등록순↓</label>
				<label><input type="radio" name="productSort" value="categoryC_desc" />카테고리↓</label> 
				<label><input type="radio" name="productSort" value="price1_desc" />가격순↓</label> 
				<label><input type="radio" name="productSort" value="readCnt_desc" />조회순↓</label>
			</span>
		</div>
		<table summary="상품 리스트" class="admin_table00">
			<!-- class="boardlist"-->
			<thead>
				<tr>
			...
			<tbody>
				<tr class='trClass'>
					...
					<td><span class='product_img_list'>상품코드</span></td>
					<td><span class='product_img_list'></span></td>
					<td class="left" style="width: 150px; word-break: break-all;">
						[<span class="p_code_list"></span>] <br /> <span class='pointer'><span
							class='p_name_list'></span></span><br />
					<span class="optionYN_list"></span>
					</td>
					<td><select class="admin_select_year" name="useType">
							<option	value="Y">승인</option>
							<option value="N">미승인</option>
							<option value="P">품절</option>
						</select> <br />
					<input type="button" name="codeplus" value="코드추가"	class="clickbutton09" /></td>
					<td><span class="prime_price_list"></span><br /> <span
						class="price_list"></span></td>
					<td><input type="text" name="readCnt" class="readCnt_list"	value="{{readCnt}}" />
						<input type="hidden" name="p_code_list"	value="{{p_code}}" /></td>
